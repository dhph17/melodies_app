...
const guestProposedSongs = [
  { id: '4', title: 'Rap God', artist: 'Eminem', image: 'https://placehold.co/50x50' },
  { id: '5', title: 'Havana', artist: 'Camila Cabello', image: 'https://placehold.co/50x50' },
  { id: '6', title: 'Blinding Lights', artist: 'The Weeknd', image: 'https://placehold.co/50x50' },
];
...
const SongModal: React.FC<SongModalProps> = ({ isVisible, onClose }) => {
  ...
  const renderProposedSongItem = ({ item }: { item: typeof guestProposedSongs[0] }) => (
    <View style={styles.songItem}>
      <Image source={{ uri: item.image }} style={styles.songImage} />
      <View style={styles.songInfo}>
        <Text style={styles.songTitle}>{item.title}</Text>
  ...
  return (
    ...
    >
      <View style={styles.modalContent}>
        ...
        <Text style={styles.sectionTitle}>Find songs to add</Text>
        <FlatList
          data={songs}
        ...
        {/* New section for guest proposed songs */}
        <Text style={styles.sectionTitle}>Songs proposed by guests</Text>
        <FlatList
          data={guestProposedSongs}
          keyExtractor={(item) => item.id}
          renderItem={renderProposedSongItem}
          showsVerticalScrollIndicator={false}
          contentContainerStyle={styles.songList}
        />
      </View>
